...
logger = logging.getLogger("male-voice-agent") # Zmieniono nazwę loggera
logger.setLevel(logging.DEBUG)
# ----------------------------------------------------

load_dotenv(dotenv_path=Path(__file__).parent.parent / '.env')
logger.info("Załadowano zmienne środowiskowe z .env, jeśli plik istnieje (agent męski).")

class SimpleAgent(Agent):
    def __init__(self) -> None:
        logger.debug("Inicjalizacja SimpleAgent (głos męski)...")
        super().__init__(
            instructions=r"""
...
Szanuj granice – nie naciskaj, jeśli użytkownik nie chce mówić.
            ...
            tts=cartesia.TTS(
                ...
                voice="4ef93bb3-682a-46e6-b881-8e157b6b4388", # <--- ZMIANA NA GŁOS MĘSKI
                speed="slow", # Możesz dostosować prędkość dla głosu męskiego
                language="pl",
                emotion=["curiosity:low", "positivity:high"] # Dostosuj emocje, jeśli potrzeba
            ),
            vad=silero.VAD.load()
        )
        logger.info("SimpleAgent (głos męski) zainicjalizowany pomyślnie.")
    ...
    async def on_enter(self):
        ...
        logger.info(f"Agent męski {agent_id_str} on_enter wywołane dla sesji {session_id_str}")
        try:
            self.session.generate_reply()
            logger.info(f"Agent męski {agent_id_str} wywołał generate_reply() w on_enter dla sesji {session_id_str}")
        except Exception as e:
            logger.error(f"Agent męski {agent_id_str} - Błąd w on_enter podczas generate_reply(): {e}", exc_info=True)
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info(f"Job {job_id} (agent męski) otrzymany, łączenie z pokojem {room_name_str}")

    try:
        await ctx.connect()
        logger.info(f"Job {job_id} (agent męski) pomyślnie połączony z pokojem {room_name_str}")
    except Exception as e:
        logger.error(f"Job {job_id} (agent męski) - Błąd podczas ctx.connect(): {e}", exc_info=True)
        return

    try:
        session = AgentSession()
        logger.debug(f"Job {job_id} (agent męski) - Obiekt AgentSession stworzony: {session}")
        logger.info(f"Job {job_id} (agent męski) - Rozpoczynanie AgentSession z SimpleAgent (głos męski)...")
        await session.start(
            agent=SimpleAgent(),
            room=ctx.room
        )
        logger.info(f"Job {job_id} (agent męski) - AgentSession wystartowała pomyślnie dla pokoju {room_name_str}")
    except Exception as e:
        logger.error(f"Job {job_id} (agent męski) - Krytyczny błąd podczas inicjalizacji lub startu AgentSession: {e}", exc_info=True)
    finally:
        logger.info(f"Job {job_id} (agent męski) - Kończenie entrypoint.")
